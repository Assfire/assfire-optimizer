...
namespace assfire::optimizer {
    ...
    std::unique_ptr<Session> RouteOptimizerEngine::solve(const Task& task, const OptimizationStrategyId& optimization_strategy_id) const {
        ...
        std::shared_ptr<SolutionPublisher> solution_publisher = _solution_publisher_provider->get_solution_publisher();
        std::shared_ptr<StateManager> state_manager           = _state_manager_provider->get_state_manager();
        std::shared_ptr<ProgressTracker> progress_tracker     = _progress_tracker_provider->get_progress_tracker();
