...
namespace assfire::optimizer {
    class Vehicle {
      public:
        ...
        const GeoPoint& start_location() const {
            return _start_location;
        }
        void set_start_location(GeoPoint start_location) {
            _start_location = std::move(start_location);
        }

        const GeoPoint& end_location() const {
            return _end_location;
        }
        void set_end_location(GeoPoint end_location) {
            _end_location = std::move(end_location);
        }

        const TransportProfileId& transport_profile_id() const {
            return _transport_profile_id;
        }
        void set_transport_profile_id(TransportProfileId transport_profile_id) {
            _transport_profile_id = std::move(transport_profile_id);
        }
